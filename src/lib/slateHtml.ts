...
function serializeText(node: Text) {
  ...
  if (node.code) html = `<code${classAttr("rounded-md bg-slate-100 px-1.5 py-0.5 font-mono text-[0.95em] text-slate-900 ring-1 ring-inset ring-slate-200")}>${html}</code>`;
...
function serializeNode(node: Descendant): string {
  ...
  switch (nodeType) {
    case "paragraph":
      return `<p${classAttr("my-4 text-[0.95rem] leading-7 text-slate-900 sm:text-[1.05rem] sm:leading-8", alignClass)}>${children || "<br />"}</p>`;
    case "heading-two":
    ...
    case "block-quote":
      return `<blockquote${classAttr(
        "my-6 rounded-xl border-r-4 border-primary bg-slate-50 px-4 py-3 text-[0.95rem] italic leading-7 text-slate-700 sm:text-[1.05rem] sm:leading-8",
        alignClass
      )}>${children}</blockquote>`;
    case "bulleted-list":
      return `<ul${classAttr("my-6 list-disc space-y-2 pr-6 text-[0.95rem] leading-7 text-slate-900 marker:text-slate-400 sm:text-[1.05rem] sm:leading-8", alignClass)}>${children}</ul>`;
    case "numbered-list":
      return `<ol${classAttr("my-6 list-decimal space-y-2 pr-6 text-[0.95rem] leading-7 text-slate-900 marker:text-slate-400 sm:text-[1.05rem] sm:leading-8", alignClass)}>${children}</ol>`;
    case "list-item":
      return `<li${classAttr("leading-7 sm:leading-8")}>${children}</li>`;
    case "code-block":
      return `<pre${classAttr("my-6 overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 font-mono text-sm leading-6 text-slate-900 sm:text-[0.95rem]")}><code>${escapeHtml(
        node.children.map((n) => (Text.isText(n) ? n.text : "")).join("\n")
      )}</code></pre>`;
    default:
      return `<p${classAttr("my-4 text-[0.95rem] leading-7 text-slate-900 sm:text-[1.05rem] sm:leading-8", alignClass)}>${children}</p>`;
  }
}
